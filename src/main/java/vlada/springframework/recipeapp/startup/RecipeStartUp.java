package vlada.springframework.recipeapp.startup;

import org.springframework.context.ApplicationListener;
import org.springframework.context.event.ContextRefreshedEvent;
import org.springframework.stereotype.Component;
import vlada.springframework.recipeapp.model.*;
import vlada.springframework.recipeapp.repositories.CategoryRepository;
import vlada.springframework.recipeapp.repositories.RecipeRepository;
import vlada.springframework.recipeapp.repositories.UnitOfMeasureRepository;
...
@Component
public class RecipeStartUp implements ApplicationListener<ContextRefreshedEvent> {

    private final CategoryRepository categoryRepository;
    private final RecipeRepository recipeRepository;
    private final UnitOfMeasureRepository unitOfMeasureRepository;

    public RecipeStartUp(CategoryRepository categoryRepository, RecipeRepository recipeRepository, UnitOfMeasureRepository unitOfMeasureRepository) {
    ...
    private List<Recipe> getRecipe() {
        ...
        // get Kategorije
        Optional<Category> serbianCategoryOptional = categoryRepository.findByDescription("Serbian");
        if(!serbianCategoryOptional.isPresent()) { throw new RuntimeException("Expected Category not found"); }

        Optional<Category> englandCategoryOptional = categoryRepository.findByDescription("England");
        if(!englandCategoryOptional.isPresent()) { throw new RuntimeException("Expected Category not found"); }
        ...
        Category serbianCategory = serbianCategoryOptional.get();
        Category englandCategory = englandCategoryOptional.get();
        ...
        guacRecipe.setNotes(guacNotes);
                                                          // konstruktor bez recepta zbog metode setRecipe iz recepta
        ...
        guacRecipe.getIngredients().add(new Ingredient("Fresh leamon juice", new BigDecimal(2), tableSpoonUom));
        ...
        guacRecipe.getCategories().add(serbianCategory);
        ...
        tacosRecipe.setNotes(tacoNotes);
                                                                        ...
                                                                        // Konstruktor sa receptom bez upotrebe metode setRecipe
        ...
        tacosRecipe.getCategories().add(serbianCategory);
